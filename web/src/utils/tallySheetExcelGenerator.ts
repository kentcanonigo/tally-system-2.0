...
export const generateTallySheetExcel = (data: TallySheetResponse) => {
  ...
  pages.forEach((page, pageIndex) => {
    ...
    // Style column headers (row 5, 0-indexed)
    const colHeaderRow = 5;
    for (let col = 0; col <= headerRange.e.c; col++) {
      const cellAddress = XLSX.utils.encode_cell({ r: colHeaderRow, c: col });
      if (!worksheet[cellAddress]) continue;
      worksheet[cellAddress].s = {
        font: { bold: true },
        alignment: { horizontal: 'center' }
    ...
    // Style totals row
    const totalsRowIndex = colHeaderRow + ROWS_PER_PAGE + 1;
    for (let col = 0; col <= headerRange.e.c; col++) {
